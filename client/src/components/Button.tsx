...
function Button({ children, onClick, type = "button" }: props) {
  return (
    <div className="relative w-[100px] h-[40px]">
      <button
        className="group relative w-full h-full flex items-center justify-center cursor-pointer bg-[#6B62FF] text-white font-bold border-4 border-white outline-4 outline-black shadow-[6px_6px_0_#2F2E41] px-4 box-border overflow-hidden transition-all duration-100 ease-out active:translate-x-[4px] active:translate-y-[4px] active:shadow-none active:bg-amber-400 active:text-black active:border-black"
        type={type}
        onClick={onClick}
      >
        {/* Shimmer effect */}
